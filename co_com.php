...
if(isset($_GET['com']))
	...
	if(empty($_GET['com']))
		...
		{
		$query = "SELECT co_com.co_com_name, co_com.co_com_desc, card.city_code FROM card, co, co_com, co_com_list WHERE '$short'=co_com.co_com_id AND co_com.co_com_id=co_com_list.co_com_id AND co_com_list.co_id=co.co_id AND '$site_id'=card.site_id AND card.co_id=co.co_id AND card.card_approve='1' AND card.city_code='1' AND card.card_video='' LIMIT 1";
		...
		if(mysql_num_rows($result)==0)
			...
			while($row = mysql_fetch_array($result))
				...
				$query = "SELECT COUNT(*) FROM card, co_com_list WHERE '$site_id'=card.site_id AND '$short'=co_com_list.co_com_id AND co_com_list.co_id=card.co_id AND card.card_approve='1' AND card.city_code='1' AND card.card_video=''";
				$limit = '9999';
				echo '<div class="pages" id="top">'; echo pagination($query,$page_link,$limit); echo '</div>'; $limstring = paginationCount($query,$limit);
				echo '<div class="deck">';
				$query2 = "SELECT card.card_id, card.card_front, co.co_id, co.co_name, card.city_code FROM card, co, co_com_list WHERE '$short'=co_com_list.co_com_id AND co_com_list.co_id=co.co_id AND '$site_id'=site_id AND card.co_id=co.co_id AND card.card_approve='1' AND card.city_code AND card.card_video='' ORDER BY card.card_id DESC $limstring";
		...
		$social_network = '<hr><span style="position:relative;top:4px;margin-right:-1px;margin-top:0px;margin-bottom:0px;padding-bottom:-10px;padding-top:10px;"><a title="'.$co_com_name.'" href="http://www.facebook.com/sharer.php?s=100&p[url]='.$currentpage.'&p[images][0]=http://boernespotlights.com/assets/img/logo.png&p[title]='.$co_com_name.'&p[summary]='.$co_com_desc.'" target="_blank" style=""><img src="http://boernespotlights.com/assets/img/facebook-icon.png" alt="Share on Facebook" style="margin-top:0px;" /></a></span>
