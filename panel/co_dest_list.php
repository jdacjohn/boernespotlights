...
if(isset($_GET['add']))
	...
	if (isset($_POST['submit']))
		{
		$co_id = $_POST['co_id'];
		$co_dest_id = $_POST['co_dest_id'];
		$result = mysql_query("INSERT INTO co_dest_list (co_id, co_dest_id) VALUES ('$co_id','$co_dest_id')");
		...
		<ul><li><select name="co_dest_id"><?php
		$result = mysql_query("SELECT co_dest_id, co_dest_name FROM co_dest WHERE '$site_id'=site_id ORDER BY co_dest_id ASC") or die(mysql_error());
		while($row = mysql_fetch_array($result))
			{
			echo '<option value="' . $row['co_dest_id'] . '">' . $row['co_dest_name'] . '</option>';
			} 
		?></select></li>
...
elseif(isset($_GET['del']))
	...
	if (empty($_GET['del']))
		...
		{
		$result = mysql_query("SELECT * FROM co_dest_list WHERE '$co_dest_list_id'=co_dest_list_id") or die(mysql_error());
		while($row = mysql_fetch_array($result))
			{
			$co_id = $row['co_id'];
			$resultco = mysql_query("DELETE FROM co_dest_list WHERE '$co_dest_list_id'=co_dest_list_id");
