<?php $pagetitle = "Community"; include '../core/base.php'; include '../'.$head; check_logged(); check_access(2);
...
if(isset($_GET['add']))
	...
	if (isset($_POST['submit']))
		{
		$co_id = $_POST['co_id'];
		$co_com_id = $_POST['co_com_id'];
		$result = mysql_query("INSERT INTO co_com_list (co_id, co_com_id) VALUES ('$co_id','$co_com_id')");
		...
		<fieldset><legend>Add Community to <?php
		...
		<ul><li><select name="co_com_id"><?php
		$result = mysql_query("SELECT co_com_id, co_com_name FROM co_com WHERE '$site_id'=site_id ORDER BY co_com_id ASC") or die(mysql_error());
		while($row = mysql_fetch_array($result))
			{
			echo '<option value="' . $row['co_com_id'] . '">' . $row['co_com_name'] . '</option>';
			} 
		?></select></li>
...
elseif(isset($_GET['del']))
	...
	if (empty($_GET['del']))
		...
		{
		$result = mysql_query("SELECT * FROM co_com_list WHERE '$co_com_list_id'=co_com_list_id") or die(mysql_error());
		while($row = mysql_fetch_array($result))
			{
			$co_id = $row['co_id'];
			$resultco = mysql_query("DELETE FROM co_com_list WHERE '$co_com_list_id'=co_com_list_id");
